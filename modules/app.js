...
// ... (DOM Elements and Mode Selectors same as before) ...
...
// ... (Rest of logic: getActiveNotes, updateFretboards, init, updateDisplay, renderNoteButtons... NO CHANGES here) ...
...
function updateFretboards(chordNotes, chordRoot = null) {
    currentActiveChordNotes = chordNotes; currentActiveChordRoot = chordRoot;
    const scaleNotes = getActiveNotes();
    let gTuning = ['E','A','D','G','B','E']; if (TUNINGS[tuningSelect.value]) gTuning = TUNINGS[tuningSelect.value].notes; else if (TUNINGS.standard) gTuning = TUNINGS.standard.notes;
    let bTuning = ['E','A','D','G']; if (TUNINGS[bassTuningSelect.value]) bTuning = TUNINGS[bassTuningSelect.value].notes; else if (TUNINGS.bass_standard) bTuning = TUNINGS.bass_standard.notes;
...
function init() {
    getNotes().forEach(note => { const option = document.createElement('option'); option.value = note; option.textContent = note; keySelect.appendChild(option); }); keySelect.value = 'C'; 
    for (const [key, value] of Object.entries(SCALES)) { const option = document.createElement('option'); option.value = key; option.textContent = value.name; scaleSelect.appendChild(option); } scaleSelect.value = 'major';
    for (const [key, value] of Object.entries(TUNINGS)) { const option = document.createElement('option'); option.value = key; option.textContent = value.name; if (value.notes.length === 6) tuningSelect.appendChild(option); else if (value.notes.length === 4) bassTuningSelect.appendChild(option); } tuningSelect.value = 'standard'; bassTuningSelect.value = 'bass_standard';
    ...
    if(playBtn) { playBtn.addEventListener('click', () => { const notes = getActiveNotes(); playScaleSequence(notes); keyboard.highlightNotes(notes, "Scale Preview", notes[0]); }); }
...
function updateDisplay() {
    ...
    let currentTuningNotes = ['E','A','D','G','B','E']; if(TUNINGS[tuningSelect.value]) currentTuningNotes = TUNINGS[tuningSelect.value].notes;
    chordRenderer.render(chordsList, capoValue, currentTuningNotes, (notes) => {
            ...
            if (clickedChord) { updateFretboards(notes, clickedChord.root); const index = chordsList.indexOf(clickedChord); chordRenderer.highlightChord(index); } else { updateFretboards(notes, notes[0]); }
        }, keySelect.value 
    );
}
function renderNoteButtons(scaleNotes) {
    if(!noteButtonsDisplay) return; noteButtonsDisplay.innerHTML = ''; const intervals = ['R', '2', '3', '4', '5', '6', '7']; let currentOctave = 3; let lastNoteIndex = -1;
    scaleNotes.forEach((note, index) => {
        const noteIndex = getNoteIndex(note); if (noteIndex < lastNoteIndex) currentOctave++; lastNoteIndex = noteIndex; const buttonOctave = currentOctave;
        const btn = document.createElement('div'); btn.className = 'note-btn'; btn.dataset.note = note; if (index === 0) btn.classList.add('root');
        const noteName = document.createElement('span'); noteName.className = 'note-name'; noteName.textContent = note;
        const noteInterval = document.createElement('span'); noteInterval.className = 'note-interval'; noteInterval.textContent = (scaleNotes.length === 7) ? intervals[index] : '';
        btn.appendChild(noteName); btn.appendChild(noteInterval);
        btn.addEventListener('click', () => { playSingleNote(note, buttonOctave); keyboard.highlightNotes([note], note, note); updateFretboards([note], note); chordRenderer.clearHighlights(); btn.style.transition = "transform 0.1s, border-color 0.1s"; btn.style.borderColor = "#00e5ff"; btn.style.transform = "scale(1.05)"; setTimeout(() => { btn.style.borderColor = ""; btn.style.transform = ""; }, 200); });
